...
describe('OrdersService', () => {
    ...
    const mockOrderRepository = {
        ...
        save: jest.fn().mockImplementation(createDto => Promise.resolve({ id: Date.now(), ...createDto })),
        findOne: jest.fn().mockImplementation(({
            where: { id },
            relations: { status, orderItems: { product } }
        }) => Promise.resolve({ id, ...mockOrderWithoutId })),
        find: jest.fn().mockImplementation(({
            where: { userId },
            relations: { status, orderItems: { product } }
        }) => Promise.resolve([{ id: Date.now(), ...mockOrderWithoutId, userId, user: { ...mockUserWithoutId, id: userId } }, { id: Date.now(), ...mockOrderWithoutId, userId, user: { ...mockUserWithoutId, id: userId } }]))
    }
    const mockStatusesService = {
        getByValue: jest.fn().mockImplementation(statusValue => Promise.resolve({ value: statusValue, id: Date.now(), description: statusValue }))
    }
    const mockUsersService = {
        getById: jest.fn().mockImplementation(userId => Promise.resolve({
            id: userId,
            ...mockUserWithoutId
        }))
    }
    const mockOrderItemsService = {
        create: jest.fn().mockImplementation((order, createOrderItemDto) => Promise.resolve({ id: Date.now(), ...createOrderItemDto })),
        removeAllByOrderId: jest.fn().mockImplementation(orderId => Promise.resolve())
    }
